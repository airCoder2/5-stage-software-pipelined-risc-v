...
begin
    ...
    -- Required data memory signals
    s_DMemWr   <= -- active high data memory write enable signal
    s_DMemAddr <= -- data memory address input
    s_DMemData <= -- data memory data input
    s_DMemOut  <= -- data memory output

    -- Required register file signals 
    s_RegWr     <= -- active high write enable input to the register file
    s_RegWrAddr <= -- destination register address input
    s_RegWrData <= -- data memory data input

    -- Required instruction memory signals
    s_IMemAddr  <= -- Do not assign this signal, assign to s_PC instead
    s_PC        <=           -- instruction memory address input.
    s_Inst      <= -- instruction signal 
    ...
    IMem: mem 
        ...
        port map(clk  => iCLK,              -- Clock
                 addr => s_IF_ID_input.PC,  -- PC is the address
    ...
    Register_file_inst: Register_file
        port map(
                 ...
                 REG_RST_IN        => iRST,
                 WRITE_SEL_IN      => s_IF_ID_output.Inst(11 downto 7),
