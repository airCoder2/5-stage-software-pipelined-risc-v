...
-- Description : This file implements a software pipelined risc-v processor 
...
architecture structure of RISCV_Processor is
    ...
    -- This component takes ALU flags and decides if the processor should branch
    ...
    -- pipline related signals

    signal s_stall : std_logic;  -- 1 if should be stalled (always 0 in software_piplined)

    -- fetch/decode signals
    -- 64 bits total:
    -- PC      : current PC (32 bits)
    -- Inst    : instruction (32 bits)
    signal s_Fetch_decode_data_in  :  std_logic_vector(63 downto 0);
    signal s_Fetch_decode_data_out : std_logic_vector(63 downto 0);
    ...
    -- Control : ALU_src
    -- Control : ALU_A_src
    -- Control : ALU_op (2 bits)
    -- Control : lui
    -- Control : mem_WE
    -- PC      : current PC (32 bits)
    -- DATA    : read 1 (32 bits)
    -- DATA    : read 2 (32 bits)
    -- DATA    : extended_imm (32 bits)
    -- Inst    : func3 & func7 (10 bits)
    ...
    -- Control : ALU/Mem
    -- Control : mem_WE
    -- ALU_FLG : Alu_branch_flag (5 bits)
    -- PC      : New branc PC (32 bits)
    -- DATA    : ALU_out (32 bits)
    -- DATA    : read 2 (32 bits)
    -- Inst    : func3 (3 bits)
    signal s_Execute_memory_data_in  : std_logic_vector(108 downto 0);   
    signal s_Execute_memory_data_out : std_logic_vector(108 downto 0);   
...
begin
    ...
    -- fetch/decode signals
    -- 64 bits total:
    IF_ID_register: N_bit_register 
        generic map(N => 64,
                    Reset_value => (others => '0'))
        port map(i_CLK => iCLK, 			          -- Clock input
                 i_RST => iRST,                       -- Reset input
                 i_WE  => not s_stall,                -- write if we are not stalling it 
                 i_D   => s_Fetch_decode_data_in,     -- fetch pipeline register data input
                 o_Q   => s_Fetch_decode_data_out);   -- fetch pipleine register data output


    -- decode/execute stage register
    -- 149 bits total:
    ID_EX_register: N_bit_register 
        generic map(N => 149,
                    Reset_value => (others => '0'))
        port map(i_CLK => iCLK,                       -- Clock input
                 i_RST => iRST,                       -- Reset input
                 i_WE  => not s_stall,                -- write if we are not stalling it 
                 i_D   => s_Decode_execute_data_in,  
                 o_Q   => s_Decode_execute_data_out);


    -- execute/memory stage register
    -- 109 bits total:
    EX_MEM_register: N_bit_register 
        generic map(N => 109,
                    Reset_value => (others => '0'))
        port map(i_CLK => iCLK, 			         -- Clock input
                 i_RST => iRST,                      -- Reset input
                 i_WE  => not s_stall,               -- write if we are not stalling it 
                 i_D   => s_Execute_memory_data_in,   
                 o_Q   => s_Execute_memory_data_out); 


    -- memory/write-back stage register
    -- 66 bits total:
    MEM_WB_register: N_bit_register 
        generic map(N => 66,
                    Reset_value => (others => '0'))
        port map(i_CLK => iCLK, 			          -- Clock input
                 i_RST => iRST,                       -- Reset input
                 i_WE  => not s_stall,                -- write if we are not stalling it 
                 i_D   => s_Memory_wback_data_in,   
                 o_Q   => s_Memory_wback_data_out); 
    ...
    oALUOut    <= s_ALU_out; -- so it doesn't optimize it
